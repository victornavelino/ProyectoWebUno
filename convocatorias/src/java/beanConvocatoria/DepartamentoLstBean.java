...
public class DepartamentoLstBean {
    ...
    public void cargarSIByProvincia(Provincia p) throws Exception{
        this.setLstDepartamento(departamentoRNLocal.findDepartamentosByProvincia(p));
        this.setLstSIDepartamento(new ArrayList<SelectItem>());
        for(Departamento d : this.getLstDepartamento()){
            this.getLstSIDepartamento().add(new SelectItem(d, d.getDescripcion()));
        }//fin for
    }
