...
public class LocalidadLstBean implements Serializable {
    ...
    public void setLstSILocalidad(List<SelectItem> lstSILocalidad) {
     ...
     public void cargarSIByLocalidad(Departamento d) throws Exception{
        this.setLstLocalidad(localidadRNLocal.findLocalidadesByDpto(d));
        this.setLstSILocalidad(new ArrayList<SelectItem>());
        for(Localidad l: this.getLstLocalidad()){
            this.getLstSILocalidad().add(new SelectItem(l, l.getDescripcion()));
        }//fin for
    }
